...
use crate::error::{length_mismatch_error, out_of_range_error, ConversionError};
use crate::{
	kind, Array, Bytes, Datetime, Duration, File, Geometry, Kind, Number, Object, Range, RecordId,
	Set, SurrealNone, SurrealNull, Table, Uuid, Value,
};
...
impl_surreal_value!(
	() as kind!(null),
	(value) => matches!(value, Value::None | Value::Null),
	(self) => Value::Null,
	(value) => {
...
impl_surreal_value!(
	<T> Option<T> as kind!(null | (T::kind_of())),
	is_option<T>(value) => matches!(value, Value::None | Value::Null) || T::is_value(value),
	from_option<T>(self) => self.map(T::into_value).unwrap_or(Value::Null),
